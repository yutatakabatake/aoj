...
let rec has_same_dices dices n =
  match n with
  | 0 -> false
  | 1 -> has_same_dices (List.tl dices) (List.length (List.tl dices))
  | _ ->
      let dice1 = List.hd dices in
      let dice2 = List.nth dices (n - 1) in
      let res = judge_same_dice dice1 dice2 in
      if res then res else has_same_dices dices (n - 1)

let rec judge_same_dices dices = has_same_dices dices (List.length dices)

let rec reader n lines : string list =
  if n == 0 then List.rev lines
  else
    let line = read_line () in
    reader (n - 1) (line :: lines)

let main () =
  let n = read_int () in
  let lines = reader n [] in
  let dices = List.map init_dice lines in
  let res = judge_same_dices dices in
  if res then print_endline "No" else print_endline "Yes"
